...
class Intra3D(Dataset):
    def __init__(self, train_mode='train', cls_state=True, npoints=2048, data_aug=True, choice=0, poison_path=None, expand=1.0):
        ...
        if (self.poison_path is not None) and (self.train_mode == 'train'):
            ...
            self.poison = self.poison * expand
        ...
        BASE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "IntrA/")

        if self.cls_state:
            self.label[0] = glob.glob(BASE + "generated/vessel/ad/" + "*.ad")
            self.label[1] = glob.glob(BASE + "generated/aneurysm/ad/" + "*.ad") + \
                            glob.glob(BASE + "annotated/ad/" + "*.ad")
    ...
    def __getitem__(self, index):
        ...
        return (point_set, seg, index) if not self.cls_state else (point_set, cls, index)
