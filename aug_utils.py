...
def cutmix_r(data_batch,cfg):
    ...
    if cfg.AUG.BETA > 0 and r < cfg.AUG.PROB:
        ...
        data_batch['pc'] = data_batch['pc'].cuda()
...
def mixup(data_batch,cfg):
    ...
    ass = ass.long()
    data_minor = data_minor.cuda()
    mix_rate_expand_xyz = mix_rate_expand_xyz.cuda()
    for i in range(batch_size):
...
def pgd(cfg, clip_func, data_batch, model, task, loss_name, dataset_name, step=10, eps=0.1, alpha=0.02, label_bias=False, num_classes=40):
    model.eval()
    ...
    for _ in range(step):
        ...
        adv_data_batch['index'] = data_batch['index']
        model.zero_grad()
        loss = get_loss(cfg, task, loss_name, adv_data_batch, out, dataset_name, label_bias=label_bias, num_classes=num_classes)
        loss.backward()
        with torch.no_grad():
...
def poison_pgd(cfg, clip_func, data_batch, poison, model, task, loss_name, dataset_name, step=7, eps=0.08, alpha=-0.015,
               class_wise=False, label_bias=False):
    orig_data = data_batch['pc'].cuda()
    ...
    for _ in range(step):
        ...
        if class_wise:
            out = model(**{'pc': orig_data + delta[label]})
            adv_data_batch['pc'] = orig_data + delta[label]
        else:
            out = model(**{'pc':orig_data + delta})
            adv_data_batch['pc'] = orig_data + delta
        adv_data_batch['label'] = data_batch['label']
        adv_data_batch['index'] = data_batch['index']
        model.zero_grad()
        loss = get_loss(cfg, task, loss_name, adv_data_batch, out, dataset_name, label_bias=label_bias)
        loss.backward()
        with torch.no_grad():
...
def nt_xent(x, y, t=0.5, reduction='mean'):
    ...
    for i in range(len(x)):
        ...
        temp1 = x[i, index]
        temp2 = x[i]
        raw_result = temp1.sum() / temp2.sum()
        results.append(-torch.log(raw_result))
...
def fc_pgd(data_batch, clip_func, poison, model, step=7, eps=0.08, alpha=0.015, class_wise=False, temperature=0.1, label_bias=False):
    orig_data = data_batch['pc'].cuda()
    index = data_batch['index']
    label = data_batch['label'].cuda()
    if label_bias:
    ...
    for _ in range(step):
        delta.requires_grad=True
        if class_wise:
            features = model(**{'pc': orig_data + delta[label]})['feature']
        else:
            features = model(**{'pc':orig_data + delta})['feature']
        model.zero_grad()
        label = label.cpu()
        loss = nt_xent(features, label, t=temperature)
        loss.backward()
        with torch.no_grad():
            delta = delta + alpha * delta.grad.sign()
            delta = delta.transpose(1, 2).contiguous()
            delta.data = clip_func(delta.clone().detach())
            delta = delta.transpose(1, 2).contiguous()
    return delta.detach()


def chamf_ce_pgd(poison, data_batch, model, adv_func, dist_func, clip_func, step=20, attack_lr=0.015,
               class_wise=False, label_bias=False, beta=1.0, threshold=-1.0, expand_coeff=1.1, errmin=False, beta_coef=None, num_classes=40):
    import torch.optim as optim
    if beta_coef is not None:
        beta_coef = torch.tensor(beta_coef).cuda()

    data = data_batch['pc']
    target = data_batch['label']
    if label_bias:
        target = (target + 3) % num_classes
    index = data_batch['index']
    if class_wise:
        delta = poison.clone()
    else:
    ...
    if (threshold >= 0.0) and (beta_coef is not None):
        if class_wise:
            adv_data = data + delta[target]
        else:
            adv_data = data + delta
        logits = model(**{'pc': adv_data.transpose(1, 2)})['logit']
        adv_loss = adv_func(logits, target)
        assert len(adv_loss) == B
        adv_loss = torch.tensor(adv_loss)
        thre = torch.kthvalue(adv_loss, min(int(threshold * len(adv_loss)) + 1, len(adv_loss)))[0]

        for i in range(len(adv_loss)):
            if adv_loss[i] < thre:
                beta_coef[index[i]] = expand_coeff * beta_coef[index[i]]
            else:
                beta_coef[index[i]] = (1 / expand_coeff) * beta_coef[index[i]]

        beta_coef = torch.clamp(beta_coef, 0.01 * beta, 1. * beta)
    ...
    for _ in range(step):
        ...
        logits = model(**{'pc': adv_data.transpose(1, 2)})['logit']  # [B, num_classes]
        ...
        if beta_coef is not None:
            ...
            adv_loss = adv_func(logits, target).mean()
        ...
        if beta_coef is not None:
            dist_loss = beta_coef[index] * dist_loss
            if label_bias or errmin:
                loss = sum(adv_loss) / len(adv_loss) + dist_loss.mean() * K
            else:
                loss = sum(adv_loss) / len(adv_loss) - dist_loss.mean() * K
        else:
            if label_bias or errmin:
                loss = adv_loss + beta * dist_loss.mean() * K
            else:
                loss = adv_loss - beta * dist_loss.mean() * K
        ...
        with torch.no_grad():
            if label_bias or errmin:
                delta = delta - attack_lr * delta.grad.sign()
            else:
                delta = delta + attack_lr * delta.grad.sign()
            delta.data = clip_func(delta.clone().detach(), normal)

    delta = delta.transpose(1, 2).contiguous()
    if beta_coef is not None:
        return delta.detach(), beta_coef.cpu()
    else:
        return delta.detach(), None


def chamf_fc_pgd(poison, data_batch, model, dist_func, clip_func, step=20, attack_lr=0.015,
               class_wise=False, label_bias=False, beta=1.0, temperature=0.1, threshold=-1.0,
               expand_coeff=10.0, beta_coef=None, num_classes=40):
    if beta_coef is not None:
        beta_coef = torch.tensor(beta_coef).cuda()
    ...
    if (threshold >= 0.0) and (beta_coef is not None):
        if class_wise:
            ...
            adv_data = data + delta
        features = model(**{'pc': adv_data.transpose(1, 2)})['feature']
        target = target.cpu()
        adv_loss = nt_xent(features, target, t=temperature, reduction='none')
        with torch.no_grad():
            assert len(adv_loss) == B
            adv_loss = torch.tensor(adv_loss)
            thre = torch.kthvalue(adv_loss, min(int(threshold * len(adv_loss)) + 1, len(adv_loss)))[0]
    ...
    for _ in range(step):
        ...
        if beta_coef is not None:
            ...
            adv_loss = nt_xent(features, target, t=temperature, reduction='mean')
...
def label_smoothing(targets, epsilon, c, num_classes=10):
    onehot = torch.eye(num_classes)[targets]
    dirich = torch.from_numpy(np.random.dirichlet(np.ones(num_classes), targets.size(0)))
...
def average_cosine_similarity(x, eps=1e-8):
    n = x.norm(p=2, dim=1, keepdim=True)
    cos_sim = (x @ x.t()) / (n * n.t()).clamp(min=eps)
