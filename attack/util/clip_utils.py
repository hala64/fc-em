...
class ClipPointsLinfDelta(nn.Module):
    ...
    def __init__(self, budget):
        ...
        Args:
            budget (float): perturbation budget
        """
        super(ClipPointsLinfDelta, self).__init__()
    ...
    def forward(self, diff):
        ...
        """
        #try:
        #    B, K = diff.shape[0], diff.shape[1]
        #    diff = diff.resize(B, 3, K)
        #except:
        #    B, K = diff.shape[0], diff.shape[2]
        #    diff = diff.resize(B, 3, K)
        #print(diff)
        with torch.no_grad():
            norm = torch.sum(diff ** 2, dim=1) ** 0.5  # [B, K]
...
class ProjectInnerPointsDelta(nn.Module):

    def __init__(self):
        """Eliminate points shifted inside an object.
        Introduced by AAAI'20 paper.
        """
        super(ProjectInnerPointsDelta, self).__init__()

    def forward(self, diff, normal=None):
        """Clipping "inside" points to the surface of the object.
...
class ProjectInnerClipLinfDelta(nn.Module):
    ...
    def __init__(self, budget):
        ...
        Args:
            budget (float): l_inf norm budget
        """
        super(ProjectInnerClipLinfDelta, self).__init__()

        self.project_inner = ProjectInnerPointsDelta()
        self.clip_linf = ClipPointsLinfDelta(budget=budget)
    ...
    def forward(self, diff, normal=None):
        ...
        with torch.no_grad():
            # project
            diff = self.project_inner(diff, normal)
            # clip
            diff = self.clip_linf(diff)
        return diff