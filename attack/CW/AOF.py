...
class CWAOF:
    ...
    def __init__(self, model, adv_func, dist_func, attack_lr=1e-2,
                 binary_step=2, num_iter=200, GAMMA=0.5, low_pass=100, clip_func=None, pa = 0.5, ps=0.5):
        ...
        self.binary_step = 1 #binary_step
    ...
    def attack(self, data, target):
        ...
        for binary_step in range(self.binary_step):
            ...
            for iteration in range(self.num_iter):
                ...
                lfc_logits = self.model(**{'pc':lfc.transpose(1, 2)})['logit']
                ...
                # print
                with torch.no_grad():
                    flogits = self.model(**{'pc':adv_data.transpose(1, 2)})['logit']  # [B, num_classes]
                    if isinstance(flogits, tuple):  # PointNet
                        flogits = flogits[0]
                    pred = torch.argmax(flogits, dim=1)  # [B]

                    lfc_flogits = self.model(**{'pc':lfc.transpose(1, 2)})['logit']
